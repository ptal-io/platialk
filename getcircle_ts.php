<?php
	...
	while($row = pg_fetch_object($result)) {
		...
		$sig = floatval($row->s);
		$query2 = "SELECT st_asgeojson(st_buffer((ST_MinimumBoundingRadius(st_collect(geom))).center,(ST_MinimumBoundingRadius(st_collect(geom))).radius)) as json, st_area(st_transform(st_buffer((ST_MinimumBoundingRadius(st_collect(geom))).center,(ST_MinimumBoundingRadius(st_collect(geom))).radius),3857)) as area FROM (SELECT a.pid, a.geom, st_distance(a.geom, b.geom) as dist FROM points a, (select geom from points where pid = ".$qid." AND id = ".$id.") b, temppopsigs c WHERE a.id = ".$id." AND c.cat = a.cat AND c.sig[".$ts."] >= ".$sig." ORDER BY dist LIMIT ".$k.") c";
